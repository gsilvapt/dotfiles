...
1. **Create `plan.md`** in the working directory
...
```
Loop 1: Create plan.md with goal and phases
...
```bash
Read plan.md  # Refresh goals in attention window
```

**After each phase:**
```bash
Edit plan.md  # Mark [x], update status
```
...
Never start a complex task without `plan.md`. This is non-negotiable.
